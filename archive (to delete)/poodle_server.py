...
def startServer(port):
  ...
  server.bind(addr)
  server.listen(5)    # param: backlog queue
  ...
  while True:
    ...
    while True:
      receive_data = connection.recv(bufsize)
...
if __name__ == '__main__':
  ...
  if sys.argv[1] == '-server':
    if len(sys.argv) != 3:
      print 'poodle_basic.py -server <port>'
      print 'poodle_basic.py -client <IP> <port> <secret cookie contents>'
      sys.exit(0)
    port = int(sys.argv[2])
    startServer(port)
